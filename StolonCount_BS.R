...
str(X)
dim(X)
hist(X$Stcount,breaks=20)

no_nas <- is.na(X$Stcount)==FALSE
X_noNA <- X[no_nas,]
dim(X_noNA)   #n=149
mean(X_noNA$Stcount)
str(X_noNA)
...
############## lm with sqrt(Stcount):
lm1 <- lm(terms(sqrt(Stcount)~
...
plot(lm1)
############## same with asreml:
asreml1 <- asreml.nvc(fixed=sqrt(Stcount)~
...
asreml2 <- asreml.nvc(fixed=sqrt(Stcount)~
                        GrassF+WithinF+GrassF:WithinF+
                        Dad+Cross+Reproduction+Dad:Reproduction+Cross:Reproduction+
                        Dad:GrassF+Cross:GrassF+Reproduction:GrassF+
                        Dad:WithinF+Cross:WithinF+Reproduction:WithinF
...
asreml3 <- asreml.nvc(fixed=sqrt(Stcount)~
                        GrassF+WithinF+GrassF:WithinF+
