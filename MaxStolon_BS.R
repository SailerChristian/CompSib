...
plot(lm1)
############## same with asreml:
asreml1 <- asreml.nvc(fixed=Maxstolon~
...
asreml2 <- asreml.nvc(fixed=Maxstolon~
                        GrassF+WithinF+GrassF:WithinF+
                        Dad+Cross+Reproduction+Dad:Reproduction+Cross:Reproduction+
                        Dad:GrassF+Cross:GrassF+Reproduction:GrassF+
                        Dad:WithinF+Cross:WithinF+Reproduction:WithinF
...
asreml3 <- asreml.nvc(fixed=Maxstolon~
                        GrassF+WithinF+GrassF:WithinF+
...
# Test Grass:Within:
asreml3.pv <- predict(asreml3,classify="GrassF:WithinF",sed=T)
asreml3.pv$predictions
# Test Within:Reproduction:
asreml3.pv <- predict(asreml3,classify="WithinF:Reproduction",sed=T)
asreml3.pv$predictions
